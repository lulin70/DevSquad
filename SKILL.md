...
7. **代码地图生成**: 自动生成项目代码结构映射，帮助架构师和开发者了解项目整体结构
...
12. **七阶段标准工作流程**: 需求分析→架构设计→测试设计→任务分解→开发实现→测试验证→发布评审
13. **跨角色设计评审机制**: PRD 评审、架构评审、测试计划评审、开发计划评审
14. **基于文档的任务分解**: 所有角色基于文档进行任务分解，确保文档驱动开发
