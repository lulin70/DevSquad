...
- **scripts/collaboration/llm_backend.py** — `OpenAIBackend.generate()` now logs a WARNING when the provider returns `finish_reason="length"` with an empty content field, printing the model and `max_tokens`. Reasoning models such as `deepseek-flash` emit their chain of thought in a separate `reasoning_content` field, but those tokens still count against `max_tokens`; once the budget is spent the response carries an empty `content`, and the method handed back `""` as if it were a valid answer (so did `FallbackBackend`, which treats `""` as success). Raw probe: `max_tokens=100` → `finish='length' content_len=0 reasoning_len=489 reasoning_tokens=100`. **Report-only by decision (user ruling 2026-09-22): the return value is deliberately unchanged** — raising the budget silently or retrying would hide the budget problem instead. Two tests cover it (warning fires / stays quiet when the truncated answer is non-empty); a negative control that silences only this logger turns the first one red. Note this cannot be caught by CI: `tests/external/test_real_llm.py` is the only test that reaches a real provider and it runs only when a provider key is supplied.
