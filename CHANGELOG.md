...
- **pyproject.toml** — `[tool.mypy] python_version` `"3.10"` → `"3.12"`. Required by the flag removal: mypy now parses the installed stubs, and `numpy >= 2.5` ships `numpy/__init__.pyi` with a PEP 695 `type` statement (line 737) that only parses for a `>= 3.12` target — under 3.10 the lint job died with `error: Type statement is only supported in Python 3.12 and greater [syntax]` (numpy reaches the lint environment as a transitive dependency of the explicitly installed `streamlit`; measured `numpy 2.5.1`, `Required-by: pandas, pydeck, streamlit`). Per-module `numpy.*` overrides were measured **not** to suppress it (`follow_imports = "skip"` and `ignore_errors = True` both still errored); the target bump does. Accepted trade-off: mypy no longer enforces 3.10-level language compatibility for project code, while `requires-python` stays `>= 3.10`. Streamlit stubs stay live under the new target, verified by `reveal_type(st.selectbox(...))` → `str`.
...
- **CI-equivalent two-segment re-run after the perf-budget + mypy-gate batch** (this host, keys blanked) —
  ...
  **e2e + integration segment**: `pytest tests/e2e/ tests/integration/ -q --tb=short --timeout=300 -m "not slow"` → `1627 passed, 4 skipped, 81 deselected, 1 warning in 390.33s (0:06:30)`, **0 failed**.
  Gates alongside (local reproduction of every blocking CI step): `ruff check . --ignore=E501` → `All checks passed!` (exit 0); `radon cc scripts/ -nd -s` → no output (0 D+); `mypy scripts/collaboration/ --ignore-missing-imports --no-error-summary` → exit 0; `mypy scripts/ skills/ --ignore-missing-imports --no-error-summary` → exit 0, `error_count=0`; `check_version_consistency.py` → `73 passed, 4 skipped, 0 failed`; `check_module_activation.py` → `8/8 production-verified, 27 self-call verified`; `check_dependency_lock.py` → `All PyPI-backed hooks aligned`; `check_test_quality.py --fail-on major` → `0 MAJOR / 373 MINOR`; `check_hidden_content.py` → 0 findings.
