...
- **Version**: Single source of truth in `_version.py` (`4.6.1`)
...
# Quick smoke test
python3 scripts/cli.py --version    # 4.6.1
...
修改 `skill-manifest.yaml` 或 `SKILL.md` 后**必须同步全部 3 层**，否则对应工作区的 TRAE 技能面板不会更新（V4.6.1 用户裁决：三层同步，任意工作区全局可用优先于面板去重）。
