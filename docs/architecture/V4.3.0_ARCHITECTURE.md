...
## 9. SDLC 用户故事驱动的 3 个新模块（V4.3.0 增量合并）

> **来源**: [SDLC 评审共识](../analysis/2026-07-25_user_stories_review_consensus.md) §3.2 Skill 调用链集成矩阵
> **合并理由**: 用户决策"合并为 V4.3.0 统一 PRD"（2026-07-25）
> **核心约束**: 防幽灵功能 — 每个新模块必须明确 Skill 调用链集成点

### 9.1 `deployment_compliance_checker.py`（新增 — Phase 0 P0-3）

**定位**: P10 部署阶段 lifecycle gate 的合规性检查器（防违规部署兜底）。

- **做什么**: 实现 `lifecycle_gate_check(phase="P10", target_env: str) -> ComplianceReport` API；规则集（简化版）：基础版禁止云端部署 / 专业版仅允许 47.116.219.15 / nginx 默认 server 必须服务官网静态文件；集成到 `unified_gate_engine.py` 作为 P10 gate checker；违规部署阻断并返回结构化报告。
- **不做什么**: 不修改 `OperationClassifier`（复用而非修改）；不实现完整 IaC 扫描（V4.4.0 扩展）；不替代 `PermissionGuard`（互补关系，PermissionGuard 管操作权限，本模块管部署目标合规性）。
- **Skill 调用链**: lifecycle gate → dispatcher 自动调用 → Markdown 报告"部署合规"章节 + 阻断违规部署
- **历史教训**: project_memory 2026-07-12 基础版违规部署事故根因是"部署前未检查硬约束"

### 9.2 `dependency_hallucination_checker.py`（新增 — Phase 1 P1-7）

**定位**: SecuritySkill 扩展，校验 AI 生成代码中的 import 语句对应真实存在的 PyPI/npm 包。

- **做什么**: 实现 `security_scan_dependencies(code: str) -> DependencyScanResult` API；复用 `InputValidator` 模式匹配提取 import 语句；三级分类（KNOWN_GOOD / UNKNOWN / SUSPICIOUS）；集成到 `skills/security/handler.py` 作为 SecuritySkill 扩展；集成到 `dispatch_hooks.py` post-worker hook（P8 实现阶段触发）；可疑包报告写入 Markdown 报告"安全检查"章节。
- **不做什么**: 不调用 PyPI/npm API 实时验证（V4.4.0 扩展，V4.3.0 用本地已知包列表 + 启发式检测）；不修改 `InputValidator`（复用而非修改）；不阻断 dispatch（默认 non-blocking，仅报告）。
- **Skill 调用链**: SecuritySkill 扩展 → `security_scan_dependencies(code)` → P8 post-worker hook 自动触发 → Markdown 报告"安全检查"章节
- **威胁模型**: CSA 2026 报告 20% AI 代码引用幻觉包，攻击者抢注恶意包

### 9.3 `output_validator.py`（升级 — Phase 2 P1-8）

**定位**: V4.1.2 Phase 2 骨架升级为生产级，dispatch pipeline post-worker hook 自动触发。

- **做什么**: 从骨架升级为完整实现；复用 `InputValidator` 53 模式反向应用（检测 LLM 输出中的 prompt injection / 敏感信息泄露 / 路径泄露 / 代码注入）；从 non-blocking 升级为可配置 blocking 模式（`.devsquad.yaml` 配置 `output_validation.mode: non_blocking|blocking`）；集成到 `dispatch_hooks.py` post-worker hook；findings 写入审计日志。
- **不做什么**: 不修改 `InputValidator`（复用而非修改）；不替代 `VerificationGate`（互补关系，VerificationGate 管"完成声明证据"，本模块管"输出内容安全"）；不实现 LLM-based 语义检测（V4.4.0 扩展）。
- **Skill 调用链**: dispatch pipeline post-worker hook → dispatcher 自动调用 → Markdown 报告"输出验证"章节 + 审计日志
- **现状**: `scripts/collaboration/output_validator.py` L1-L60 标注 "V4.1.2 Phase 2 skeleton, deferred to Phase 3"，本 P1-8 完成该升级
...
用户任务 → [InputValidator] → [RoleMatcher] → [Coordinator]
                                    ...
                                    [Worker 并行执行]
                                              ↓
                                    [post-worker hooks] ← SDLC 新模块集成点 1
                                      ├─ OutputValidator（自动触发，P1-8）
                                      ├─ DependencyHallucinationChecker（SecuritySkill 调用，P1-7）
                                    ...
                                    [post-dispatch hooks]
                                              ↓
                                    [lifecycle gate: P10 部署] ← SDLC 新模块集成点 2
                                      └─ DeploymentComplianceChecker（P0-3）
                                              ↓
...
| 验证维度 | 验证方法 | CI 检查 |
|---------|---------|---------|
| Skill 调用链存在 | 每个新模块在 `dispatcher.py` 或 `unified_gate_engine.py` 中有显式注册 | `check_skill_integration.py` 检查模块在 dispatcher 公共 API 中暴露 |
| 模块被调用 | 每个新模块有 `feature_usage_tracker` 计数 | `check_module_activation.py` 检查最近 30 天调用次数 > 0 |
| 用户可见性 | 每个新模块在 Markdown 报告中有专属章节 | E2E 测试验证报告包含对应章节 |
| 测试覆盖 | 每个新模块单元测试覆盖率 ≥80% + 至少 1 个 E2E 测试 | pytest --cov 检查覆盖率 |

### 9.6 与 V4.3.0 原 4 个新模块的依赖关系

| SDLC 新模块 | 依赖 V4.3.0 原模块 | 依赖关系 |
|-----------|------------------|---------|
| `deployment_compliance_checker.py` | `unified_gate_engine.py` | 集成为 P10 gate checker |
| `dependency_hallucination_checker.py` | `input_validator.py` / `operation_classifier.py` / `dispatch_hooks.py` / `skills/security/handler.py` | 复用模式匹配 + 集成到 SecuritySkill + post-worker hook |
| `output_validator.py`（升级） | `dispatch_hooks.py` / `input_validator.py` | 集成到 post-worker hook + 复用 53 模式 |

### 9.7 暂缓到 V4.4.0 的模块

| 模块 | 暂缓理由 | V4.3.0 占位 |
|------|---------|-----------|
| `benchmark_regression_checker.py` | 依赖 nightly CI 基础设施增强，工作量大 | E2E-08 测试骨架 xfail |
...
| 2026-07-25 | v1.1 | 合并 SDLC 共识方案 3 个新模块（P0-3/P1-7/P1-8）；新增 §9 SDLC 模块架构章节；新增 Skill 调用链集成图示；新增防幽灵功能验证机制；新增与原 4 模块的依赖关系；用户决策"合并为 V4.3.0 统一 PRD" | DevSquad Architect |
