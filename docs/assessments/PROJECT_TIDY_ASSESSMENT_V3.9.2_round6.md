...
**可维护性维度**:
- `mcp_server.py` 3 处硬编码 "3.7.0" + 模块数/测试数过期（48/1548 → 70/2853）

**文档维度**:
- CHANGELOG-CN 缺 2026-06-29 P0 安全修复条目 → 已补充
- 模块数口径不一致（70+/118+/149）→ mcp_server.py 已统一为 70
- 测试数口径不一致（2703/2600+/3084）→ mcp_server.py 已统一为 2853
...
| mcp_server.py 模块数/测试数 | ✅ 48→70, 1548→2853 |
| 版本一致性 15/15 | ✅ 全通过 |
| CN/JP README 缺 Python API 方法 | ⏳ P2 延期（不阻塞发布） |
| CN README "3种使用方式" vs 4 种 | ⏳ P2 延期 |
...
| 文档 | 8.5 | 8.5 | — | CHANGELOG-CN 补充，README P2 延期 |
