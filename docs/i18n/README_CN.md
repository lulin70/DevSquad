...
python3 scripts/cli.py --version       # 显示版本 (3.7.0)
```
...
```bash
# 快速健康检查（应在 30 秒内完成）
python3 scripts/cli.py --version       # 预期输出: DevSquad 3.7.0
python3 scripts/cli.py status          # 预期输出: 系统就绪
python3 scripts/cli.py roles           # 预期输出: 列出 7 个核心角色

# 完整测试套件
python3 -m pytest tests/ -q --tb=line # 预期输出: 2109 passed
```
