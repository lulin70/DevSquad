...
class TestStructuredReportFormat(unittest.TestCase):
    ...
    def test_structured_report_has_summary_section(self):
        ...
        has_summary = (
            "任务" in report or "Task" in report or len(result.summary) > 0
    ...
    def test_structured_report_has_role_table(self):
        ...
        has_roles = (
            "角色" in report or "Role" in report or len(result.matched_roles) > 0
            if isinstance(result, DispatchResult)
            else True
        )
...
class TestCompactReportFormat(unittest.TestCase):
    ...
    def test_compact_report_has_status_icon(self):
        ...
        has_status = (
            "✅" in report
            or "❌" in report
            or (isinstance(result, DispatchResult) and (result.success or not result.success))
        )
    ...
    def test_compact_report_has_task_info(self):
        ...
        is_valid = len(report) > 30 and (isinstance(result, DispatchResult) and result.task_description)
