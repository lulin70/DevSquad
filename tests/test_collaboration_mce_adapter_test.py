...
class TestMCEResult(unittest.TestCase):
    ...
    def test_custom_values(self):
        result = MCEResult(
            memory_type="knowledge", confidence=0.92, tier="tier3", metadata={"carrymem_type": "fact_declaration"}
        )
...
class TestMCEAdapterThreadSafety(unittest.TestCase):
    ...
    def test_concurrent_classify_disabled(self):
        ...
        threads = [threading.Thread(target=classify_worker, args=(i,)) for i in range(10)]
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=5)
...
class TestNormalizeResult(unittest.TestCase):
    ...
    def test_normalize_carrymem_format(self):
        raw = {
            ...
            "entries": [{"type": "user_preference", "confidence": 0.85, "tier": 2, "content": "dark mode"}],
        }
    ...
    def test_normalize_carrymem_decision(self):
        raw = {
            "should_remember": True,
            "entries": [{"type": "decision", "confidence": 0.92, "tier": 3}],
        }
        result = MCEAdapter._normalize_result(raw)
