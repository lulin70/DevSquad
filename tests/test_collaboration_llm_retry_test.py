...
class TestRetryConfig:
    ...
    def test_custom_config(self):
        ...
        config = RetryConfig(max_retries=5, initial_delay=2.0, max_delay=120.0, exponential_base=3.0, jitter=False)
...
class TestLLMRetryManager:
    ...
    def test_successful_retry(self):
        ...
        result = self.manager.retry_with_fallback(
            mock_func, args=(), kwargs={}, config=config, fallback_backends=None, current_backend="test"
        )
    ...
    def test_retry_on_failure(self):
        ...
        mock_func = Mock(side_effect=[Exception("503 Service Unavailable"), Exception("Connection timeout"), "success"])
    ...
    def test_all_backends_fail(self):
        ...
        # 注意: retry_with_fallback 会包装异常消息
        with pytest.raises(Exception, match="All fallback backends failed"):
    ...
    def test_statistics_tracking(self):
        ...
        self.manager.retry_with_fallback(mock_func1, (), {}, config, None, "backend1")

        # Failed call with retry
        mock_func2 = Mock(side_effect=[Exception("503 Error"), "success"])
        self.manager.retry_with_fallback(mock_func2, (), {}, config, None, "backend2")
...
class TestRetryDecorator:
    ...
    def test_decorator_with_fallback(self):
        ...
        @retry_with_fallback(max_retries=2, initial_delay=0.01, fallback_backends=["openai", "anthropic"])
        def test_func(prompt: str, backend: str = "primary"):
            if backend == "primary":
                raise Exception("Primary down")
            # Fallback backends 应该成功
