...
def t3_03_add_rule():
    g = PermissionGuard()
    r = PermissionRule("CUSTOM", ActionType.FILE_CREATE, "*.log", PermissionLevel.BYPASS, "custom rule")
    g.add_rule(r)
...
def t3_11_no_match_fallback():
    ...
    d = g.check(a)
    assert_true(
        d.outcome in (DecisionOutcome.ALLOWED, DecisionOutcome.PROMPT, DecisionOutcome.DENIED),
        "fallback decision valid",
    )
...
def t3_18_duplicate_rule_id_overwrites():
    ...
    r = PermissionRule("R001", ActionType.FILE_READ, "*", PermissionLevel.BYPASS, "overwrite")
    g.add_rule(r)
...
def t4_03_medium_risk_ops():
    ...
    found_medium = False
    for t in [
        "modify src/main.py with new feature",
        "create output.json for data export",
...
def t5_10_security_report_structure():
    ...
    keys = ["total_checks", "allowed", "denied", "prompted", "escalated", "avg_risk_score", "top_denied_actions"]
    for k in keys:
...
def t6_01_empty_action():
    ...
    assert_true(
        d.outcome in (DecisionOutcome.ALLOWED, DecisionOutcome.DENIED, DecisionOutcome.PROMPT),
        "valid outcome for empty",
    )
...
def t6_14_many_rules_perf():
    many_rules = [
        PermissionRule(f"CUST-{i}", ActionType.FILE_READ, f"pattern_{i}", PermissionLevel.DEFAULT, f"rule {i}")
        for i in range(500)
    ]
...
def it1_02_full_workflow():
    ...
    assert_true(any(o == DecisionOutcome.PROMPT for o in outcomes), "at least one prompt for .env")
...
def it2_01_escalated_detected():
    ...
    assert_true(
        d.outcome in (DecisionOutcome.DENIED, DecisionOutcome.PROMPT, DecisionOutcome.ESCALATED),
...
def it2_02_scratchpad_conflict_simulation():
    ...
    caught = [
        ...
        for e in g.get_audit_log()
        if e.decision
        and e.decision.outcome in (DecisionOutcome.DENIED, DecisionOutcome.PROMPT, DecisionOutcome.ESCALATED)
    ]
...
def e2e_03_journey_c_incident_investigation():
    ...
    report = g.get_security_report()
    assert_true(
        report["denied"] >= 1 or report["prompted"] >= 1,
...
def e2e_05_five_roles_with_guard():
    ...
    for role in roles:
        for i in range(3):
            ...
            d = g.check(
                make_action(at, target=f"{role}_output_{i}.md", source_role_id=role, source_worker_id=f"{role}-worker")
            )
