...
def t3_02_snip_removes_low_importance():
    ...
    assert_true(len(low_msgs) == 0 or len(result.messages) < len(msgs), "Low importance messages should be removed")
...
def t3_03_snip_reduces_token_count():
    c = ContextCompressor(thresholds={CompressionLevel.SNIP: 100})
    msgs = make_msgs(
        ["short message filler text"] * 30
        + ["important architecture decision about system design and implementation details here"] * 5
    )
    result = c.check_and_compress(msgs, force_level=CompressionLevel.SNIP)
    assert_true(result.compressed_token_count < result.original_token_count, "SNIP should reduce token count")
...
def t3_04_snip_extracts_memory_from_snipped():
    ...
    assert_true(
        len(memory) >= 1,
        f"Snipped messages should produce memory entries, got {len(memory)}, kept {len(result.messages)}/{len(msgs)}",
    )
...
def t5_03_fullcompact_sections_present():
    ...
    assert_true(has_decisions or has_todos or len(result.summary) > 50, "Summary should contain categorized sections")
...
def t5_04_fullcompact_high_compression_ratio():
    ...
    result = c.check_and_compress(msgs, force_level=CompressionLevel.FULL_COMPACT)
    assert_true(
        result.reduction_percent > 80, f"FullCompact should have >80% reduction, got {result.reduction_percent:.1f}%"
    )
...
def t6_02_at_snip_threshold():
    ...
    result = c.check_and_compress(msgs)
    assert_true(
        result.compression_level in (CompressionLevel.SNIP, CompressionLevel.SESSION_MEMORY, CompressionLevel.NONE),
...
def t6_03_force_level_override():
    ...
    msgs = make_msgs(["tiny"])
    result = c.check_and_compress(msgs, force_level=CompressionLevel.FULL_COMPACT)
    assert_eq(result.compression_level, CompressionLevel.FULL_COMPACT, "Force level should override auto detection")
...
def t8_02_get_by_category():
    ...
    assert_true(len(decisions) >= 1 or len(errors) >= 1, "Category filtering should work")
...
def t9_02_single_message():
    ...
    result = c.check_and_compress(msgs, force_level=CompressionLevel.SNIP)
    assert_true(
        isinstance(result, CompressedContext) and len(result.messages) >= 0, "Single message should be processed"
    )
...
def t12_02_realistic_workflow():
    ...
    assert_true(
        result.compressed_token_count <= result.original_token_count + 1,
