...
class TestIntentDetectionEnglish:
    ...
    def test_detect_bug_fix_english(self):
        result = self.mapper.detect_intent("Fix the login page bug that causes crash", lang="en")
        assert result is not None
        assert result.intent_type == "bug_fix"

    def test_detect_new_feature_english(self):
        result = self.mapper.detect_intent("Implement a new user authentication feature", lang="en")
        assert result is not None
        assert result.intent_type == "new_feature"
        assert "architect" in result.required_roles

    def test_detect_security_review_english(self):
        result = self.mapper.detect_intent("Check for SQL injection vulnerabilities and harden", lang="en")
        assert result is not None
        assert result.intent_type == "security_review"
        assert result.gate == "owasp_checklist"

    def test_detect_code_review_english(self):
        result = self.mapper.detect_intent("Review this code for quality issues", lang="en")
        assert result is not None
        assert result.intent_type == "code_review"

    def test_detect_performance_english(self):
        result = self.mapper.detect_intent("Optimize API response time, it's too slow", lang="en")
        assert result is not None
        assert result.intent_type == "performance_optimization"
        assert result.gate == "measure_first"

    def test_detect_deployment_english(self):
        result = self.mapper.detect_intent("Deploy to production with CI/CD pipeline", lang="en")
        assert result is not None
        assert result.intent_type == "deployment"
...
class TestIntentDetectionJapanese:
    ...
    def test_detect_bug_fix_japanese(self):
        result = self.mapper.detect_intent("ログインページのバグを修正してください", lang="ja")
        assert result is not None
        assert result.intent_type == "bug_fix"

    def test_detect_new_feature_japanese(self):
        result = self.mapper.detect_intent("新しいユーザー認証機能を実装する", lang="ja")
        assert result is not None
        assert result.intent_type == "new_feature"

    def test_detect_security_review_japanese(self):
        result = self.mapper.detect_intent("SQLインジェクションの脆弱性をチェックして強化", lang="ja")
        assert result is not None
        assert result.intent_type == "security_review"

    def test_detect_performance_japanese(self):
        result = self.mapper.detect_intent("APIレスポンスが遅い、パフォーマンスを改善", lang="ja")
        assert result is not None
        assert result.intent_type == "performance_optimization"

    def test_detect_deployment_japanese(self):
        result = self.mapper.detect_intent("本番環境にデプロイ", lang="ja")
        assert result is not None
        assert result.intent_type == "deployment"
...
class TestConfidenceScoring:
    ...
    def test_high_confidence_multiple_keywords(self):
        ...
        result = mapper.detect_intent("Fix bug error fail crash exception problem", lang="en")
        assert result is not None
    ...
    def test_confidence_capped_at_one(self):
        ...
        result = mapper.detect_intent("fix bug error fail crash broken issue defect problem exception", lang="en")
        assert result is not None
...
class TestCachingMechanism:
    ...
    def test_cache_key_includes_language(self):
        ...
        assert result_zh is not result_en or (result_zh is None and result_en is None)
...
class TestEdgeCases:
    ...
    def test_special_characters_in_input(self):
        mapper = IntentWorkflowMapper(confidence_threshold=0.1)
        result = mapper.detect_intent("Fix bug! @#$% Error: 404 (not found)", lang="en")
        assert result is not None
