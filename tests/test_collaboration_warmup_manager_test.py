...
class T3EagerWarmup(unittest.TestCase):
    ...
    def test_01_single_success(self):
        ...
        wm._tasks.clear()
        wm._results.clear()
        task = WarmupTask(task_id="e1", name="E1", layer=WarmupLayer.EAGER, executor=lambda: "hello", timeout_ms=1000)
        wm.register_task(task)
    ...
    def test_02_dependency_order(self):
        ...
        wm.register_task(WarmupTask(task_id="a", name="A", layer=WarmupLayer.EAGER, executor=make_executor("a")))
        wm.register_task(
            WarmupTask(task_id="b", name="B", layer=WarmupLayer.EAGER, dependencies=["a"], executor=make_executor("b"))
        )
        wm.register_task(
            WarmupTask(task_id="c", name="C", layer=WarmupLayer.EAGER, dependencies=["b"], executor=make_executor("c"))
        )
        wm.warmup_eager()
    ...
    def test_03_exception_handling(self):
        ...
        wm._tasks.clear()
        wm._results.clear()
        task = WarmupTask(task_id="err", name="Err", layer=WarmupLayer.EAGER, executor=lambda: 1 / 0, timeout_ms=1000)
        wm.register_task(task)
        results = wm.warmup_eager()
    ...
    def test_06_duration_recorded(self):
        ...
        task = WarmupTask(
            ...
            name="Dur",
            layer=WarmupLayer.EAGER,
            executor=lambda: time.sleep(0.01) or "done",
            timeout_ms=5000,
        )
        wm.register_task(task)
        results = wm.warmup_eager()
    ...
    def test_07_result_cached(self):
        ...
        wm._results.clear()
        task = WarmupTask(
            task_id="cached-task", name="Cached", layer=WarmupLayer.EAGER, executor=lambda: {"data": True}
        )
        wm.register_task(task)
        wm.warmup_eager()
...
class T4AsyncWarmup(unittest.TestCase):
    ...
    def test_04_dependency_order_async(self):
        ...
        wm.register_task(WarmupTask(task_id="adep-a", name="ADepA", layer=WarmupLayer.ASYNC, executor=make_exec("a")))
        wm.register_task(
            WarmupTask(
                task_id="adep-b",
                name="ADepB",
...
class T6DependencyResolution(unittest.TestCase):
    ...
    def test_01_no_deps_preserves_order(self):
        wm = WarmupManager.instance(WarmupConfig.fast())
        tasks = [
            WarmupTask(task_id=f"z-{i}", name=f"Z{i}", layer=WarmupLayer.EAGER, executor=lambda i=i: i)
            for i in range(3)
        ]
        for t in tasks:
            wm.register_task(t)
    ...
    def test_02_chain_a_b_c(self):
        wm = WarmupManager.instance(WarmupConfig.fast())
        tasks = [
            WarmupTask(task_id="c", name="C", layer=WarmupLayer.EAGER, dependencies=["b"], executor=lambda: "c"),
            WarmupTask(task_id="b", name="B", layer=WarmupLayer.EAGER, dependencies=["a"], executor=lambda: "b"),
            WarmupTask(task_id="a", name="A", layer=WarmupLayer.EAGER, executor=lambda: "a"),
        ]
        for t in tasks:
            wm.register_task(t)
    ...
    def test_03_diamond_dep(self):
        wm = WarmupManager.instance(WarmupConfig.fast())
        tasks = [
            WarmupTask(task_id="root", name="Root", layer=WarmupLayer.EAGER, executor=lambda: "r"),
            WarmupTask(
                task_id="left", name="Left", layer=WarmupLayer.EAGER, dependencies=["root"], executor=lambda: "l"
            ),
            WarmupTask(
                task_id="right", name="Right", layer=WarmupLayer.EAGER, dependencies=["root"], executor=lambda: "ri"
            ),
            WarmupTask(
                task_id="bottom",
                name="Bottom",
    ...
    def test_04_circular_detects(self):
        wm = WarmupManager.instance(WarmupConfig.fast())
        tasks = [
            WarmupTask(task_id="x", name="X", layer=WarmupLayer.EAGER, dependencies=["y"], executor=lambda: None),
            WarmupTask(task_id="y", name="Y", layer=WarmupLayer.EAGER, dependencies=["x"], executor=lambda: None),
        ]
        for t in tasks:
            wm.register_task(t)
    ...
    def test_05_self_dep(self):
        wm = WarmupManager.instance(WarmupConfig.fast())
        task = WarmupTask(
            task_id="self", name="Self", layer=WarmupLayer.EAGER, dependencies=["self"], executor=lambda: None
        )
        wm.register_task(task)
...
class T8EdgeCasesAndDegradation(unittest.TestCase):
    ...
    def test_02_timeout_handling(self):
        ...
        task = WarmupTask(task_id="tout", name="TOut", layer=WarmupLayer.EAGER, executor=slow_op, timeout_ms=50)
        wm.register_task(task)
        results = wm.warmup_eager()
        elapsed = (time.perf_counter() - start) * 1000
    ...
    def test_09_large_timeout_value(self):
        ...
        wm._results.clear()
        task = WarmupTask(
            task_id="bigt", name="BigTimeout", layer=WarmupLayer.EAGER, executor=lambda: "big", timeout_ms=999999
        )
        wm.register_task(task)
        results = wm.warmup_eager()
        self.assertEqual(results[0].status, WarmupStatus.SUCCESS)
