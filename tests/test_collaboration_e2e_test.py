...
def test_scratchpad_write_read():
    from scripts.collaboration import EntryType, Scratchpad, ScratchpadEntry
...
def test_scratchpad_resolve():
    from scripts.collaboration import EntryStatus, EntryType, Scratchpad, ScratchpadEntry
...
def test_scratchpad_conflicts():
    ...
    sp = Scratchpad()
    sp.write(ScratchpadEntry(worker_id="w1", role_id="architect", entry_type=EntryType.CONFLICT, content="技术方案A"))
...
def test_scratchpad_summary():
    ...
    sp = Scratchpad()
    sp.write(ScratchpadEntry(worker_id="w1", role_id="architect", entry_type=EntryType.FINDING, content="发现1"))
    sp.write(ScratchpadEntry(worker_id="w2", role_id="tester", entry_type=EntryType.DECISION, content="决策1"))
...
def test_worker_execute():
    from scripts.collaboration import Scratchpad, TaskDefinition, Worker
...
def test_worker_write_finding():
    from scripts.collaboration import EntryType, Scratchpad, ScratchpadEntry, Worker
...
def test_consensus_create_proposal():
    ...
    ce = ConsensusEngine()
    p = ce.create_proposal("是否采用微服务架构？", "coord-1", "建议使用微服务架构以获得更好的扩展性")
...
def test_consensus_vote_and_decide():
    ...
    ce.cast_vote(
        p.proposal_id, Vote(voter_id="arch-1", voter_role="architect", decision=True, weight=1.5, reason="技术可行")
    )
    ce.cast_vote(
        p.proposal_id, Vote(voter_id="pm-1", voter_role="product-manager", decision=True, weight=1.2, reason="业务支持")
    )
...
def test_consensus_veto():
    from scripts.collaboration import ConsensusEngine, DecisionOutcome, Vote
    ...
    ce.cast_vote(
        p.proposal_id, Vote(voter_id="sec-1", voter_role="security", decision=False, weight=-1.0, reason="安全风险")
    )  # veto!
    record = ce.reach_consensus(p.proposal_id)
...
def test_coordinator_collect_results():
    ...
    coord = Coordinator()
    plan = coord.plan_task("收集结果测试", [{"role_id": "architect"}, {"role_id": "tester"}])
...
def test_coordinator_generate_report():
    ...
    coord = Coordinator()
    plan = coord.plan_task("生成报告测试", [{"role_id": "architect"}, {"role_id": "tester"}])
    coord.spawn_workers(plan)
    coord.execute_plan(plan)
...
def test_batch_scheduler_parallel():
    from scripts.collaboration import BatchMode, BatchScheduler, TaskBatch, TaskDefinition
