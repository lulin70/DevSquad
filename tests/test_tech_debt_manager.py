...
class TestGetDebtReport:
    ...
    def test_report_by_severity(self):
        mgr = TechDebtManager()
        mgr.identify_debt("tester", DebtCategory.CODE_QUALITY, "Low", "a.py", severity=DebtSeverity.LOW)
        mgr.identify_debt("tester", DebtCategory.SECURITY, "Critical", "b.py", severity=DebtSeverity.CRITICAL)
        report = mgr.get_debt_report()
    ...
    def test_report_top_priority(self):
        mgr = TechDebtManager()
        mgr.identify_debt(
            "tester", DebtCategory.CODE_QUALITY, "Low", "a.py", severity=DebtSeverity.LOW, effort=DebtEffort.EPIC
        )
        mgr.identify_debt(
            "tester",
            DebtCategory.SECURITY,
    ...
    def test_report_interest_forecast(self):
        mgr = TechDebtManager()
        mgr.identify_debt("tester", DebtCategory.SECURITY, "Issue", "a.py", severity=DebtSeverity.HIGH)
        report = mgr.get_debt_report()
        assert len(report.interest_forecast) == 6

    def test_report_remediation_progress(self):
        mgr = TechDebtManager()
        mgr.identify_debt("tester", DebtCategory.CODE_QUALITY, "Active", "a.py")
