...
def agent_briefing(temp_storage_dir):
    ...
    briefing = AgentBriefing(
        agent_role="Architect", project_context={"name": "DevSquad", "version": "3.6.7"}, storage_dir=temp_storage_dir
    )
    yield briefing
...
def test_agent_briefing_initialization(agent_briefing):
    ...
    assert agent_briefing.project_context["version"] == "3.6.7"
...
def test_generate_briefing_basic(agent_briefing):
    ...
    briefing = agent_briefing.generate_briefing(task="Design Protocol interface system", context={"priority": "high"})
...
def test_generate_briefing_with_capabilities(agent_briefing):
    ...
    briefing = agent_briefing.generate_briefing(task="Design REST API", context={})
...
def test_generate_briefing_with_constraints(agent_briefing):
    ...
    briefing = agent_briefing.generate_briefing(task="Write code", context={})
...
def test_generate_briefing_max_length(agent_briefing):
    ...
    # Add lots of content
    for i in range(10):
        agent_briefing.add_section(title=f"Section {i}", content="A" * 1000, priority=2)

    # Generate briefing with max length
    briefing = agent_briefing.generate_briefing(task="Test task", context={}, max_length=500)
...
def test_update_briefing_with_section(agent_briefing):
    ...
    agent_briefing.update_briefing(
        key="decision", value="Use Python Protocol", section="Technical Decisions", priority=1
    )
...
def test_add_section(agent_briefing):
    ...
    agent_briefing.add_section(
        title="Best Practices", content="Always use type hints", priority=1, metadata={"category": "coding"}
    )
...
def test_persistence_save_and_load(temp_storage_dir):
    ...
    briefing1 = AgentBriefing(agent_role="Developer", project_context={"name": "Test"}, storage_dir=temp_storage_dir)
    briefing1.add_section("Section 1", "Content 1")
    briefing1.update_briefing("capabilities", "Coding")

    # Create new instance (should load from disk)
    briefing2 = AgentBriefing(agent_role="Developer", storage_dir=temp_storage_dir)
...
def test_briefing_section_dataclass(agent_briefing):
    ...
    section = BriefingSection(title="Test", content="Content", priority=1, metadata={"key": "value"})
