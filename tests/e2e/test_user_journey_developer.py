...
class TestE2EUserJourneyDeveloperOnboarding:
    ...
    def test_uj1_1_installation_verification(self, e2e_runner):
        ...
        assert "3.6" in version_output or "devsquad" in version_output, f"Version output unexpected: {version_output}"
    ...
    def test_uj1_2_quick_init_creates_config(self, e2e_runner, temp_project_dir):
        ...
        assert "unknown" not in result.stderr.lower(), f"Init command not recognized: {result.stderr}"

        # Check if config file was created or already exists
        config_file = temp_project_dir / ".devsquad.yaml"
        temp_project_dir / ".env"
    ...
    def test_uj1_3_first_task_execution(self, e2e_runner):
        ...
        result = e2e_runner.run_cli_command(
            [
                ...
                "-t",
                "Design a user authentication REST API with JWT tokens",
                "--roles",
                "architect",
                "coder",
                "test",
        ...
        assert len(result.stdout) > 50, f"Output too short, may indicate issue: {result.stdout}"
    ...
    def test_uj1_4_view_system_status_and_roles(self, e2e_runner):
        ...
        assert any(keyword in status_output for keyword in ["ready", "version", "system", "status", "devsquad"]), (
    ...
    def test_uj1_5_help_and_documentation_access(self, e2e_runner):
        ...
        expected_commands = ["dispatch", "demo", "status", "roles", "init", "run", "help", "version"]
        ...
        try:
            dispatch_help = e2e_runner.run_cli_command(["dispatch", "--help"], expect_success=False)
    ...
    def test_uj1_6_error_handling_for_invalid_input(self, e2e_runner):
        ...
        for scenario in error_scenarios:
            ...
            found_error_indicator = any(err_msg in combined_output for err_msg in scenario["expected_error"])

            # Either got proper error message OR command was rejected
            assert (
                result.returncode != 0
                or found_error_indicator
    ...
    def test_uj1_7_lifecycle_commands_discovery(self, e2e_runner):
        ...
        for phase_cmd, _ in lifecycle_phases:
            try:
                ...
                result = e2e_runner.run_cli_command(
                    ["lifecycle", phase_cmd, "--help"], expect_success=False, timeout=10
                )
        ...
        assert len(available_phases) >= 2, f"Expected at least 2 lifecycle commands, found: {available_phases}"

        print(f"✅ Lifecycle commands available: {', '.join(available_phases)}")

    def test_uj1_8_configuration_file_validation(self, e2e_runner, temp_project_dir):  # noqa: ARG002
        """
        ...
        custom_config = {
            "quality_control": {"enabled": True, "strict_mode": True, "min_quality_score": 90},
            "llm": {"backend": "mock", "timeout": 60},
            "logging": {"level": "INFO"},
        }
        ...
        try:
            ...
            assert loaded_config.get("quality_control", {}).get("strict_mode")

            print("✅ Config validation passed: strict_mode=True")
