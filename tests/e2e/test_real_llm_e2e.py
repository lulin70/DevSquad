...
def _has_api_key() -> bool:
    ...
    return bool(os.environ.get("DEVSQUAD_OPENAI_API_KEY")) or bool(os.environ.get("DEVSQUAD_ANTHROPIC_API_KEY"))
...
def _run_dispatch(backend: str = "mock", extra_env: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, str(_CLI_PATH), "dispatch", "-t", "What is 2+2?", "-f", "markdown", "--dry-run"],
...
def test_e2e_dispatch_completes_via_cli():
    ...
    assert len(mock_output) > 0, f"Empty output from mock dispatch: {mock_output[:100]}"
    ...
    if _has_api_key():
        for backend in ["openai", "anthropic"]:
            ...
            if result.returncode in (0, 1):
                ...
                assert len(output) > 0, f"Empty output from {backend}: {output[:100]}"
...
def test_e2e_dispatch_response_format_markdown():
    ...
    assert result.returncode in (0, 1), f"dispatch --dry-run failed: {result.stderr[:300]}"
...
def test_e2e_dispatch_response_format_compact():
    ...
    result = subprocess.run(
        [sys.executable, str(_CLI_PATH), "dispatch", "-t", "Design a REST API", "-f", "compact", "--dry-run"],
        cwd=str(_PROJECT_ROOT),
        capture_output=True,
        text=True,
        timeout=60,
    ...
    assert result.returncode in (0, 1), f"dispatch -f compact failed: {result.stderr[:300]}"
...
def test_e2e_unreachable_api_returns_clear_error():
    ...
    try:
        result = subprocess.run(
            [sys.executable, str(_CLI_PATH), "dispatch", "-t", "Hello", "--dry-run"],
            cwd=str(_PROJECT_ROOT),
            capture_output=True,
            text=True,
            timeout=45,
    ...
    is_clear_error = result.returncode != 0 and (
