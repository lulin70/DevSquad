...
class TestT1ScratchpadCRUD:
    ...
    def test_t1_1_write_basic(self):
        ...
        entry = ScratchpadEntry(
            worker_id="w1", role_id="arch", entry_type=EntryType.FINDING, content="发现N+1查询问题", confidence=0.9
        )
        eid = sp.write(entry)
    ...
    def test_t1_2_auto_id_timestamp(self):
        ...
        assert abs(ts_val - before) < 1.0, "timestamp within 1s of 'before'"
    ...
    def test_t1_5_resolve(self):
        """标记已解决+版本递增"""
        sp = Scratchpad()
        entry = ScratchpadEntry(worker_id="w1", role_id="dev", entry_type=EntryType.FINDING, content="待修复bug")
        eid = sp.write(entry)
...
class TestT5WorkerInteraction:
    ...
    def test_t5_5_notifications_clear(self):
        ...
        w = Worker("w1", "arch", "", sp)
        w.send_notification(
            TaskNotification(from_worker="w1", to_workers=["w2"], notification_type="info", summary="test")
        )
        notifs = w.get_pending_notifications()
        assert len(notifs) == 1
...
class TestT6ConsensusProposalVoting:
    ...
    def test_t6_2_cast_vote_normal(self):
        ...
        ce.cast_vote(p.proposal_id, Vote(voter_id="v1", voter_role="arch", decision=True, weight=1.5))
    ...
    def test_t6_3_repeat_voting(self):
        ...
        p = ce.create_proposal("t", "c1", "c")
        for i in range(3):
            ce.cast_vote(p.proposal_id, Vote(voter_id=f"v{i}", voter_role="arch", decision=True, weight=1.0))
...
class TestT7ConsensusDecisionOutcomes:
    ...
    def test_t7_1_simple_majority_approve(self):
        ...
        ce.cast_vote(p.proposal_id, Vote(voter_id="p1", voter_role="product-manager", decision=True, weight=1.2))
    ...
    def test_t7_2_veto_escalates(self):
        ...
        ce.cast_vote(
            p.proposal_id, Vote(voter_id="s1", voter_role="security", decision=False, weight=-1.0, reason="安全风险")
        )
        r = ce.reach_consensus(p.proposal_id)
    ...
    def test_t7_5_super_majority(self):
        ...
        for vid, role, wgt in [
            ...
            ce.cast_vote(p.proposal_id, Vote(voter_id=vid, voter_role=role, decision=True, weight=wgt))
        ce.cast_vote(p.proposal_id, Vote(voter_id="nay1", voter_role="tester", decision=False, weight=1.0))
        r = ce.reach_consensus(p.proposal_id)
        assert r.outcome == DecisionOutcome.APPROVED
...
class TestT9CoordinatorExecuteCollect:
    ...
    def test_t9_3_collect_results_structure(self):
        ...
        for key in ["coordinator_id", "scratchpad", "scratchpad_stats", "findings_count", "workers"]:
...
class TestT10CoordinatorConflictReport:
    ...
    def test_t10_2_has_conflicts(self):
        """有冲突时发起共识"""
        coord = Coordinator()
        coord.scratchpad.write(ScratchpadEntry(entry_type=EntryType.CONFLICT, content="冲突A"))
        coord.scratchpad.write(ScratchpadEntry(entry_type=EntryType.CONFLICT, content="冲突B"))
        coord.spawn_workers(
...
class TestBoundaryException:
    ...
    def test_bt_14_all_against(self):
        ...
        p = ce.create_proposal("t", "c", "c")
        for i in range(3):
            ce.cast_vote(p.proposal_id, Vote(voter_id=f"v{i}", voter_role="arch", decision=False, weight=1.0))
        r = ce.reach_consensus(p.proposal_id)
    ...
    def test_bt_15_all_veto(self):
        ...
        p = ce.create_proposal("t", "c", "c")
        for i in range(3):
            ce.cast_vote(p.proposal_id, Vote(voter_id=f"v{i}", voter_role="sec", decision=False, weight=-1.0))
        r = ce.reach_consensus(p.proposal_id)
        assert r.outcome == DecisionOutcome.ESCALATED
...
class TestIntegration:
    ...
    def test_it_3_coord_full_chain(self):
        ...
        coord = Coordinator()
        plan = coord.plan_task("集成链路测试", [{"role_id": "architect"}, {"role_id": "tester"}])
        coord.spawn_workers(plan)
        result = coord.execute_plan(plan)
        collection = coord.collect_results()
        assert result.success is True
    ...
    def test_it_4_conflict_resolution_integration(self):
        ...
        coord = Coordinator()
        plan = coord.plan_task("冲突解决测试", [{"role_id": "architect"}, {"role_id": "security"}])
        coord.spawn_workers(plan)
        coord.scratchpad.write(ScratchpadEntry(entry_type=EntryType.CONFLICT, content="方案存在安全隐患"))
        resolutions = coord.resolve_conflicts()
    ...
    def test_it_9_reference_preserved(self):
        ...
        ref_entry = ScratchpadEntry(
            content="referencing entry", references=[Reference(ReferenceType.SUPPORTS, target, "supports target")]
        )
        sp.write(ref_entry)
        data = sp.export_json()
        parsed = json.loads(data)
...
class TestE2EUserJourneys:
    ...
    def test_e2e_2_journey_b_lightweight(self):
        ...
        e1 = sp.write(
            ScratchpadEntry(
                worker_id="dev-a", role_id="coder", entry_type=EntryType.FINDING, content="API 响应时间超过 2s"
            )
        )
    ...
    def test_e2e_3_journey_c_consensus_only(self):
        """旅程C: 独立共识决策"""
        ce = ConsensusEngine()
        p = ce.create_proposal("是否升级到 Python 3.12?", "tech-lead", "Python 3.12 提供更好的性能和类型提示支持")
        assert p.proposal_id.startswith("prop-")
        ce.cast_vote(
            p.proposal_id,
            Vote(voter_id="arch-1", voter_role="architect", decision=True, weight=1.5, reason="性能提升明显"),
        )
        ce.cast_vote(
            p.proposal_id,
            Vote(voter_id="pm-1", voter_role="product-manager", decision=True, weight=1.2, reason="生态兼容性好"),
        )
        ce.cast_vote(
            p.proposal_id,
            Vote(voter_id="dev-1", voter_role="solo-coder", decision=False, weight=1.0, reason="迁移成本高"),
        )
        record = ce.reach_consensus(p.proposal_id)
    ...
    def test_e2e_5_with_veto(self):
        ...
        ce.cast_vote(p.proposal_id, Vote(voter_id="pm-1", voter_role="pm", decision=True, weight=1.2))
        ce.cast_vote(
            p.proposal_id,
            Vote(voter_id="sec-1", voter_role="security", decision=False, weight=-1.0, reason="仍有XSS漏洞"),
        )
        record = ce.reach_consensus(p.proposal_id)
