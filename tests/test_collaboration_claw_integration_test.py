...
class TestClawCoreMemories(unittest.TestCase):
    ...
    @unittest.skipUnless(WorkBuddyClawSource().is_available, "Claw directory not available")
    def test_load_core_memories_returns_items(self):
        core = self.source._load_core_memories()
        self.assertGreaterEqual(len(core), 2, "Should have at least SOUL.md + USER.md")

    @unittest.skipUnless(WorkBuddyClawSource().is_available, "Claw directory not available")
    def test_core_memory_types_are_valid(self):
        core = self.source._load_core_memories()
        valid_types = {MemoryType.SEMANTIC, MemoryType.KNOWLEDGE, MemoryType.EPISODIC, MemoryType.PATTERN}
        for item in core:
...
class TestClawDailyMemories(unittest.TestCase):
    ...
    def test_daily_memories_exist(self):
        ...
        self.assertGreater(len(daily), 0, "Should have at least some daily log files")
...
class TestClawRecallIntegration(unittest.TestCase):
    ...
    def test_recall_with_claw_includes_claw_items(self):
        ...
        if self.bridge._claw_enabled:
            ...
            self.assertGreater(len(all_claw), 0, "Claw source should have items when enabled")
