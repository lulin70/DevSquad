...
"""
    ...
    def test_05_valid_call_no_issues(self):
        ...
        issues = self.validator.validate_call_against_signature("func", {"a", "b"}, sigs)
...
class T4_TestPurposeParser(unittest.TestCase):
    ...
    def test_04_error_dimension_inference(self):
        ...
        node, lines = self._make_func_node("test_err", body="    with self.assertRaises(ValueError):\n        bad()")
        meta = self.parser.parse_function(node, lines)
    ...
    def test_05_performance_dimension_inference(self):
        ...
        node, lines = self._make_func_node(
            "test_perf", body="    start = time.time()\n    result = func()\n    elapsed = time.time() - start"
        )
        meta = self.parser.parse_function(node, lines)
...
class T6_TemplateGeneration(unittest.TestCase):
    ...
    def test_01_basic_template_has_happy_path(self):
        ...
        sig = APISignature(name="process", kind="method", params=[{"name": "self"}, {"name": "data", "type": "str"}])
    ...
    def test_02_with_error_dimension(self):
        ...
        sig = APISignature(name="calc", kind="function", params=[{"name": "x", "type": "int"}])
        tpl = self.guard.generate_test_template(sig, dimensions=[TestDimension.HAPPY_PATH, TestDimension.ERROR_CASE])
    ...
    def test_03_with_perf_dimension(self):
        ...
        sig = APISignature(name="query", kind="method", params=[{"name": "self"}, {"name": "sql", "type": "str"}])
        tpl = self.guard.generate_test_template(sig, dimensions=[TestDimension.HAPPY_PATH, TestDimension.PERFORMANCE])
    ...
    def test_04_template_is_valid_python(self):
        ...
        sig = APISignature(name="run", kind="function", params=[{"name": "cmd", "type": "str"}])
        tpl = self.guard.generate_test_template(sig)
