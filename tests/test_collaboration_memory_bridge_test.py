...
class T1DataModels(unittest.TestCase):
    ...
    def test_03_memory_item_age_days(self):
        item = MemoryItem(id="t2", memory_type=MemoryType.EPISODIC, title="Age", content="data")
        age = item.age_days
    ...
    def test_08_recall_result_structure(self):
        r = MemoryRecallResult(
            memories=[MemoryItem(id="x", memory_type=MemoryType.KNOWLEDGE, title="T", content="C")],
            total_found=1,
            query_time_ms=5.5,
...
class T2MemoryWriter(unittest.TestCase):
    ...
    def test_07_batch_write(self):
        items = [
            MemoryItem(id=f"bw-{i}", memory_type=MemoryType.KNOWLEDGE, title=f"Batch {i}", content=f"Content {i}")
            for i in range(5)
        ]
...
class T3MemoryReader(unittest.TestCase):
    def setUp(self):
        ...
        for i in range(3):
            self.writer.write_knowledge(
                KnowledgeItem(
                    id=f"kr{i}",
                    domain="domain-A" if i < 2 else "domain-B",
                    title=f"Knowledge {i}",
                    content=f"Content {i}",
                    tags=[f"tag{i}"],
                )
    ...
    def test_05_read_feedback_by_type(self):
        self.writer.write_feedback(UserFeedback(id="rfb1", feedback_type="suggestion", content="good"))
        self.writer.write_feedback(UserFeedback(id="rfb2", feedback_type="complaint", content="bad"))
        suggestions = self.reader.read_feedback(feedback_type="suggestion")
        self.assertEqual(len(suggestions), 1)

    def test_06_read_patterns_by_category(self):
        self.writer.write_pattern(
            PersistedPattern(id="rp1", name="P1", slug="p1-slug", category="cat-a", quality_score=80)
        )
        self.writer.write_pattern(
            PersistedPattern(id="rp2", name="P2", slug="p2-slug", category="cat-b", quality_score=75)
        )
        cat_a = self.reader.read_patterns(category="cat-a")
        self.assertEqual(len(cat_a), 1)

    def test_07_read_analysis_by_status(self):
        self.writer.write_analysis(AnalysisCase(id="ra1", problem="Bug A", status="open"))
        self.writer.write_analysis(AnalysisCase(id="ra2", problem="Bug B", status="completed"))
        completed = self.reader.read_analysis_cases(status="completed")
        self.assertEqual(len(completed), 1)

    def test_08_read_patterns_all(self):
        self.writer.write_pattern(PersistedPattern(id="rpa1", name="All P", slug="all-p-slug", category="cat-x"))
...
class T4MemoryIndexer(unittest.TestCase):
    ...
    def test_01_build_index_basic(self):
        items = [
            ...
            MemoryItem(
                id="i2", memory_type=MemoryType.EPISODIC, title="数据库优化", content="索引优化技巧", domain="性能优化"
            ),
        ]
    ...
    def test_03_add_to_index_incremental(self):
        item = MemoryItem(
            id="inc1", memory_type=MemoryType.KNOWLEDGE, title="Test incremental", content="add one by one"
        )
        self.indexer.add_to_index(item)
        self.assertEqual(self.indexer.size, 1)
        self.indexer.add_to_index(
            MemoryItem(id="inc2", memory_type=MemoryType.KNOWLEDGE, title="Second item", content="another")
        )
        self.assertEqual(self.indexer.size, 2)

    def test_04_remove_from_index(self):
        item = MemoryItem(id="rem1", memory_type=MemoryType.KNOWLEDGE, title="Remove me", content="will be gone")
        self.indexer.add_to_index(item)
        self.assertEqual(self.indexer.size, 1)
    ...
    def test_05_search_exact_match(self):
        items = [
            MemoryItem(id="sm1", memory_type=MemoryType.KNOWLEDGE, title="Redis缓存使用", content="缓存穿透防护"),
            MemoryItem(id="sm2", memory_type=MemoryType.FEEDBACK, title="用户建议", content="希望增加Redis支持"),
        ]
        self.indexer.build_index(items)
    ...
    def test_06_search_partial_match(self):
        items = [
            MemoryItem(id="sp1", memory_type=MemoryType.KNOWLEDGE, title="Python异步编程", content="asyncio详解"),
            MemoryItem(id="sp2", memory_type=MemoryType.KNOWLEDGE, title="Java并发", content="线程池配置"),
        ]
        self.indexer.build_index(items)
    ...
    def test_07_search_no_results(self):
        items = [
            MemoryItem(id="snr1", memory_type=MemoryType.KNOWLEDGE, title="Database", content="SQL queries"),
        ]
        self.indexer.build_index(items)
    ...
    def test_08_search_type_filter(self):
        items = [
            MemoryItem(id="stf1", memory_type=MemoryType.KNOWLEDGE, title="知识条目", content="关于知识的内容"),
            MemoryItem(id="stf2", memory_type=MemoryType.FEEDBACK, title="反馈条目", content="关于反馈的内容"),
        ]
        self.indexer.build_index(items)
    ...
    def test_09_search_domain_filter(self):
        items = [
            MemoryItem(
                id="sdf1", memory_type=MemoryType.KNOWLEDGE, title="架构决策A", content="关于架构", domain="架构设计"
            ),
            MemoryItem(
                id="sdf2", memory_type=MemoryType.KNOWLEDGE, title="安全策略B", content="关于安全", domain="安全"
            ),
        ]
        self.indexer.build_index(items)
    ...
    def test_10_search_limit(self):
        items = [
            MemoryItem(
                ...
                memory_type=MemoryType.KNOWLEDGE,
                title=f"Item {i}",
                content=f"Content with keyword {i} keyword",
            )
            for i in range(10)
        ]
        self.indexer.build_index(items)
    ...
    def test_11_keyword_search_and_logic(self):
        items = [
            MemoryItem(id="ks1", memory_type=MemoryType.KNOWLEDGE, title="Redis缓存一致性", content="分布式锁方案"),
            MemoryItem(id="ks2", memory_type=MemoryType.KNOWLEDGE, title="MySQL主从复制", content="binlog同步"),
        ]
        self.indexer.build_index(items)
    ...
    def test_12_keyword_single_word(self):
        items = [
            MemoryItem(id="kss1", memory_type=MemoryType.KNOWLEDGE, title="Docker容器化", content="镜像构建"),
        ]
        self.indexer.build_index(items)
    ...
    def test_14_tfidf_relevance(self):
        items = [
            MemoryItem(
                id="tf1", memory_type=MemoryType.KNOWLEDGE, title="Redis缓存设计", content="Redis缓存策略和实现"
            ),
            MemoryItem(id="tf2", memory_type=MemoryType.KNOWLEDGE, title="Python基础", content="Python语法入门教程"),
        ]
        self.indexer.build_index(items)
...
class T5MemoryBridgeCore(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.bridge = MemoryBridge(base_dir=self.tmpdir, config=MemoryConfig.default())
    ...
    def test_10_learn_from_mistake(self):
        ctx = ErrorContext(error_message="ConnectionTimeoutError: 数据库连接超时", task_description="执行批量导入")
    ...
    def test_13_recent_history(self):
        for i in range(3):
            self.bridge.writer.write_episodic(
                EpisodicMemory(
                    id=f"rh{i}",
                    task_description=f"History {i}",
...
class T6LifecycleManagement(unittest.TestCase):
    ...
    def test_01_forgetting_weight_new(self):
        item = MemoryItem(id="fw1", memory_type=MemoryType.EPISODIC, title="New", content="new memory")
    ...
    def test_05_compress_old_memories(self):
        ...
        self.bridge.writer.write_episodic(
            EpisodicMemory(
                id="comp1",
                task_description="Compress test old",
                finding="A" * 500,
                created_at=old_time,
            )
        )
        ...
        self.bridge.writer.write_episodic(
            EpisodicMemory(
                id="comp2",
                task_description="Compress test new",
                finding="B" * 50,
                created_at=new_time,
            )
        )
    ...
    def test_06_compress_new_untouched(self):
        ...
        self.bridge.writer.write_episodic(
            EpisodicMemory(
                id="cnu1",
                task_description="New untouched",
                finding="New finding",
                created_at=new_time,
            )
        )
    ...
    def test_08_cleanup_config_off(self):
        ...
        bridge.writer.write_episodic(
            EpisodicMemory(
                id="coff1",
                task_description="Config off test",
...
class T8EdgeCases(unittest.TestCase):
    ...
    def test_06_corrupted_json_handling(self):
        bad_file = os.path.join(self.tmpdir, "knowledge_base", "domains", "general", "corrupt.json")
...
class E2ETests(unittest.TestCase):
    ...
    def test_03_full_lifecycle(self):
        kw = KnowledgeItem(id="lc1", domain="LC", title="Lifecycle Test", content="Full lifecycle test content")
