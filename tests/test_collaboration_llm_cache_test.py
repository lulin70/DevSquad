...
from scripts.collaboration.llm_cache import LLMCache, CacheEntry, get_llm_cache, reset_cache
...
class TestLLMCache:
    ...
    def test_lru_eviction(self, cache):
        ...
        # 注意: 由于 get() 会从磁盘恢复，这里检查内存缓存大小
        # 而不是特定条目是否存在
        assert len(cache.memory_cache) <= 3, "内存缓存不应超过 max_memory_entries"
        
        stats = cache.get_stats()
        assert stats["evictions"] >= 1, "应该至少发生过一次淘汰"
    ...
    def test_clear_old(self, cache):
        ...
        # 磁盘文件也应该被删除
        disk_files = list(cache.cache_dir.glob("*.json"))
