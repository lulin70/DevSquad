...
class TestTotalEntriesProperty(unittest.TestCase):
    ...
    def test_total_entries_sufficient_coverage(self):
        self.assertGreaterEqual(self.engine.total_entries, 15, "Must have universal entries + role-specific entries")
...
class TestIntegrationPromptAssembly(unittest.TestCase):
    ...
    def test_all_roles_produce_valid_injectable_content(self):
        for role in ["architect", "product-manager", "security", "tester", "solo-coder", "devops", "ui-designer"]:
            content = self.engine.format_for_prompt(role)
            self.assertGreater(len(content), 0, f"Role '{role}' must produce non-empty content")
