#!/usr/bin/env python3
"""
CLI Visual Enhancement Module for DevSquad V3.6.9
...
class VisualFormatter:
    ...
    def print_lifecycle_header(
        ...
        self.print_title(f"🔄 DevSquad Lifecycle [{command.upper()}]", "View Layer Mode (Plan C Architecture)")
        ...
        if mapping:
            ...
            print(self._c("  │", Colors.CYAN) + " 📋 Phases: ".ljust(15) + self._c(phases_str, Colors.GREEN))

        if preset:
            roles_str = ", ".join(preset.get("required_roles", [])[:3])
            print(self._c("  │", Colors.CYAN) + " 👥 Roles: ".ljust(15) + self._c(roles_str, Colors.BLUE))
    ...
    def print_gate_status(
        ...
        verdict_line = (
            f"    {self._c(verdict_icon, verdict_color)} Verdict: {self._c(verdict_text, Colors.BOLD + verdict_color)}"
        )
        print(verdict_line)
    ...
    def print_status_summary(
        ...
        self.print_title(
            "📊 Lifecycle Status Summary", f"Mode: {mode.value if hasattr(mode, 'value') else mode}" if mode else ""
        )
    ...
    def print_footer(self, version: str = "V3.6.9") -> None:
        ...
        footer = f"  DevSquad {version} | {timestamp} | {Icons.SPARKLE} Plan C Layered Architecture"
    ...
    try:
        ...
        vf.print_info_box(
            ...
            [
                ...
                "Or use: python3 examples/quick_start.py",
                "View guide: cat docs/USAGE_GUIDE.md",
            ],
            icon=Icons.ROCKET,
            color=Colors.GREEN,
        )
...
def _get_preset(command: str) -> dict | None:
