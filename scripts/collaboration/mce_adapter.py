...
class MCEAdapter:
    ...
    def _try_init(self):
        with self._lock:
            try:
                ...
                version = getattr(__import__("memory_classification_engine"), "__version__", "unknown")
                ...
                # DevSquadAdapter init may raise unpredictable errors from external library
                import logging
            ...
            try:
                if hasattr(self._carrymem, "match_rules"):
                    result = self._carrymem.match_rules(
                        task_description=task_description, user_id=safe_user_id, role=role, max_rules=max_rules
                    )
                else:
                    return self._keyword_fallback_match(task_description, safe_user_id, role, max_rules)
    ...
    def add_rule(
        self, trigger: str, action: str, rule_type: str = "always", confidence: float = 0.8
    ) -> dict[str, Any] | None:
        ...
        try:
            from scripts.collaboration.rule_collector import LocalRuleStorage, RuleData
    ...
    def _get_all_rules_raw(self, user_id: str, role: str | None = None) -> list[dict[str, Any]]:
    ...
    def _normalize_result(raw: Any) -> MCEResult:
        ...
        if isinstance(raw, dict):
            ...
            if entries:
                ...
                meta = {k: v for k, v in first.items() if k not in ("type", "memory_type", "confidence", "tier")}
