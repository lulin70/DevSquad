...
class AdaptiveRoleSelector:
    ...
    def select_roles(
        ...
        if similar_cases:
            roles = self._select_from_similar_tasks(
                similar_cases, min_success_rate, max_roles
            )
            if roles:
            ...
            if case.get("success", False):
                ...
                if combo_key not in successful_combos:
                    ...
                    existing["avg_duration"] = (
                        (existing["avg_duration"] * n + case.get("total_duration", 0.0))
                        / (n + 1)
                    )
                    existing["count"] += 1
                    existing["similarity"] = max(
                        existing["similarity"], case.get("similarity", 0.0)
                    )
