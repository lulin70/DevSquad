...
class RedisCacheBackend(CacheBackendInterface):
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if attempt < self.retry_attempts - 1:
                    logger.warning(
                        f"{operation_name} failed (attempt {attempt + 1}/{self.retry_attempts}): {e}"
                    )
    ...
    async def clear(self) -> None:
        ...
        async def _do_clear(client):
            ...
            logger.info(f"Cleared {total_deleted} entries from Redis")
...
class SyncRedisCacheWrapper:
    ...
    def __init__(self, redis_url: str, prefix: str = "devsquad:",
                 default_ttl: int = 3600, compression: bool = False):
