...
class ExecutionGuard:
    ...
    def __init__(
        ...
        abort_keywords: Optional[List[str]] = None,
    ):
        """
    ...
    def configure(self, trigger_name: str, value) -> None:
        ...
        logger.debug(
            "ExecutionGuard configured: %s = %s (was %s)",
            trigger_name, value, old_value
        )
    ...
    def check_abort(
        ...
        if should_abort:
            self._abort_count += 1
            logger.warning(
                "ExecutionGuard TRIGGERED (check #%d): %s",
                self._check_count, abort_reason
            )

        return should_abort, abort_reason

    def _check_timeout(self, elapsed_time: float) -> Optional[str]:
        """Check if execution has exceeded maximum duration."""
        max_duration = self._triggers["max_duration_sec"]
        if elapsed_time > max_duration:
            return (
                f"Timeout exceeded: {elapsed_time:.1f}s > {max_duration:.1f}s "
                f"(+{elapsed_time - max_duration:.1f}s)"
            )
        return None

    def _check_output_size(self, output: str) -> Optional[str]:
        """Check if output length exceeds maximum."""
        max_length = self._triggers["max_output_length"]
        if len(output) > max_length:
            return (
                f"Output too large: {len(output)} chars > {max_length} chars "
                f"(+{len(output) - max_length} chars)"
            )
        return None

    def _check_token_limit(self, token_count: int) -> Optional[str]:
        """Check if token count exceeds maximum."""
        max_tokens = self._triggers["max_output_tokens"]
        if token_count > max_tokens:
            return (
                f"Token limit exceeded: {token_count} tokens > {max_tokens} tokens "
                f"(+{token_count - max_tokens} tokens)"
            )
        return None

    def _check_critical_keywords(self, output: str) -> Optional[str]:
        """
        ...
        if matched_keywords:
            return (
                f"Critical keywords detected: {', '.join(matched_keywords[:3])}"
            )
        return None
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            "abort_rate": (
                round(self._abort_count / max(self._check_count, 1), 3)
