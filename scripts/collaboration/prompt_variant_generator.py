...
class PromptVariantGenerator:
    ...
    def generate_from_pattern(self, pattern: SuccessPattern, base_template: str = "") -> VariantGenerationResult:
        """
        从成功模式生成候选提示词变体
        ...
        if pattern.confidence < self.MIN_CONFIDENCE:
            return VariantGenerationResult(
                success=False, reason=f"置信度不足: {pattern.confidence:.2f} < {self.MIN_CONFIDENCE}"
            )

        if pattern.frequency < self.MIN_FREQUENCY:
            return VariantGenerationResult(
                success=False, reason=f"频率不足: {pattern.frequency} < {self.MIN_FREQUENCY}"
            )

        if not pattern.applicable_roles:
            return VariantGenerationResult(success=False, reason="模式未关联任何角色")
    ...
    def _assemble_variant_prompt(self, pattern: SuccessPattern, base_template: str) -> str:
        """
        从模式组装变体提示词
    ...
    def _calculate_quality(self, pattern: SuccessPattern, prompt_text: str) -> float:
        """
        计算变体质量评分 [0, 100]
        ...
        safe_steps = sum(1 for ps in pattern.steps_template if ps.estimated_risk < 0.5)
        total_steps = max(len(pattern.steps_template), 1)
        step_score = 5.0 * (safe_steps / total_steps)

        return round(confidence_score + freq_score + content_score + specificity_score + step_score, 1)
    ...
    def get_candidates_for_role(self, role_id: str) -> list[PromptVariant]:
        ...
        """
        with self._lock:
            candidates = [v for v in self._variants.values() if v.role_id == role_id and v.status == "candidate"]
    ...
    def record_feedback(self, variant_id: str, positive: bool = True) -> bool:
        """
        记录用户/AI 对变体的反馈
    ...
    def auto_deprecate(self) -> list[str]:
        ...
        with self._lock:
            for vid, variant in list(self._variants.items()):
                ...
                if (
                    variant.usage_count >= 3
                    and ratio <= self.DEPRECATION_THRESHOLD
                    or variant.created_at < cutoff
                    and variant.usage_count == 0
                ):
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "overall_feedback_ratio": round(total_positive / max(total_positive + total_negative, 1), 2),
                "avg_quality_score": round(sum(v.quality_score for v in all_vars) / max(len(all_vars), 1), 1)
                if all_vars
                else 0.0,
            }
