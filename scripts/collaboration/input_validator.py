...
class InputValidator:
    ...
    def validate_task(self, task: str) -> ValidationResult:
        ...
        if not isinstance(task, str):
            return ValidationResult(valid=False, reason=f"Task must be a string, got {type(task).__name__}")
        ...
        if task_length < self.min_length and not is_rule_command:
            return ValidationResult(
                valid=False, reason=f"Task too short (min {self.min_length} chars, got {task_length})"
            )

        if task_length > self.max_length:
            return ValidationResult(
                valid=False, reason=f"Task too long (max {self.max_length} chars, got {task_length})"
            )

        # 3. 空白检查
        if not task.strip():
            return ValidationResult(valid=False, reason="Task cannot be empty or whitespace only")
        ...
        try:
            ...
            return ValidationResult(valid=False, reason=f"Invalid UTF-8 encoding: {e}")

        # 5. 危险模式检查（使用规范化文本）
        for regex in self._forbidden_regex:
            match = regex.search(task_normalized)
            if match:
                return ValidationResult(valid=False, reason="Input contains forbidden content")
        ...
        if self.strict_mode:
            for regex in self._suspicious_regex:
                ...
                if match:
                    return ValidationResult(
                        valid=False, reason=f"Suspicious pattern detected (strict mode): {match.group(0)}"
                    )
    ...
    def validate_roles(self, roles: list[str]) -> ValidationResult:
        ...
        if not isinstance(roles, list):
            return ValidationResult(valid=False, reason=f"Roles must be a list, got {type(roles).__name__}")

        # 2. 数量检查
        if len(roles) > self.MAX_ROLE_COUNT:
            return ValidationResult(valid=False, reason=f"Too many roles (max {self.MAX_ROLE_COUNT}, got {len(roles)})")

        # 3. 空列表检查
        if not roles:
            return ValidationResult(valid=False, reason="Roles list cannot be empty")

        # 4. 每个角色的验证
        for role in roles:
            if not isinstance(role, str):
                return ValidationResult(valid=False, reason=f"Role must be a string, got {type(role).__name__}")

            if not role.strip():
                return ValidationResult(valid=False, reason="Role cannot be empty or whitespace only")

            # 角色 ID 只能包含字母、数字、连字符和下划线
            if not re.match(r"^[a-zA-Z0-9_-]+$", role):
                return ValidationResult(
                    valid=False, reason=f"Invalid role ID: {role} (only alphanumeric, -, _ allowed)"
                )
