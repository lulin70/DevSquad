...
class UsageTracker:
    ...
    def track(self, feature_name: str, success: bool = True, metadata: dict | None = None) -> None:
        """追踪功能使用
    ...
    def get_stats(self, feature_name: str | None = None) -> dict:
        """获取统计数据
    ...
    def get_error_prone_features(self, min_calls: int = 5, error_threshold: float = 0.1) -> list[tuple[str, float]]:
        """获取高错误率的功能
        ...
        with self._lock:
            ...
            if error_prone:
                ...
                for name, error_rate in error_prone[:5]:
                    stat = self.stats[name]
                    lines.append(
                        f"- **{name}**: 错误率 {error_rate * 100:.1f}% ({stat['errors']}/{stat['count']} 次调用)"
                    )
...
def track_usage(feature_name: str, success: bool = True, metadata: dict | None = None) -> None:
    """便捷函数：追踪功能使用
...
def get_usage_stats(feature_name: str | None = None) -> dict:
    """便捷函数：获取统计数据
