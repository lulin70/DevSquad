...
class AsyncLLMCache:
    ...
    async def get(self, prompt: str, backend: str, model: str) -> str | None:
        """
...
_global_async_cache: AsyncLLMCache | None = None
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        # Set cache
        await cache.set(
            prompt="What is Python?", response="Python is a programming language.", backend="openai", model="gpt-4"
        )

        # Get from cache
        response = await cache.get(prompt="What is Python?", backend="openai", model="gpt-4")
