...
class TestFunctionMeta:
    ...
    purpose_text: str = ""
    dimension: TestDimension | None = None
    assert_count: int = 0
    ...
    def to_markdown(self) -> str:
        ...
        if self.issues:
            ...
            for issue in self.issues:
                icon = {"critical": "🔴", "major": "🟠", "minor": "🟡", "info": "🔵", "suggestion": "💡"}.get(
...
class TestPurposeParser:
    ...
    DIMENSION_KEYWORDS = {
        TestDimension.ERROR_CASE: ["error", "exception", "invalid", "fail", "错误", "异常", "非法", "失败", "raises"],
        ...
        TestDimension.BOUNDARY: ["boundary", "edge", "empty", "null", "zero", "max", "min", "边界", "空", "极限"],
        TestDimension.CONFIGURATION: ["config", "setting", "option", "env", "配置", "设定", "环境变量"],
        TestDimension.INTEGRATION: ["integration", "e2e", "end.to.end", "集成", "端到端"],
        TestDimension.SECURITY: ["security", "auth", "permission", "inject", "安全", "权限", "注入"],
    }
    ...
    def parse_function(self, func_node: ast.FunctionDef, source_lines: list[str]) -> TestFunctionMeta:
        ...
        meta.has_error_test = (
            "assertRaises" in func_source
            or "except" in func_source
            or "error" in lower_source
            or "exception" in lower_source
            or "invalid" in lower_source
        )
        meta.has_performance_check = any(
            kw in lower_source for kw in ["time", "duration", "latency", "benchmark", "performance", "timing"]
        )
    ...
    def _infer_dimension(self, source: str) -> TestDimension | None:
        lower = source.lower()
        scores = {}
...
class TestQualityGuard:
    ...
    def audit(self) -> TestQualityReport:
        ...
        test_funcs = [
            node for node in ast.walk(test_tree) if isinstance(node, ast.FunctionDef) and node.name.startswith("test_")
        ]
    ...
    def audit_project(self, project_root: str, pattern: str = "**/*_test.py") -> list[TestQualityReport]:
        """审计整个项目的所有测试文件"""
        reports = []
    ...
    def generate_test_template(self, api_sig: APISignature, dimensions: list[TestDimension] = None) -> str:
        ...
        param_docs = "\n".join(
            f"        {p['name']}: {p['type'] or 'Any'}" for p in api_sig.params if p["name"] != "self"
        )
    ...
    def test_{api_sig.name}_happy_path(self):
        ...
        # Arrange
{param_docs if param_docs else "        # Placeholder: 正常输入参数（由生成器自动填充）"}
...
'''
    ...
    def test_{api_sig.name}_invalid_input(self):
        ...
        # Placeholder: 非法参数测试用例（由生成器根据API签名自动生成）
