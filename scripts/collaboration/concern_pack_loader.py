...
class ConcernPackLoader:
    ...
    def __init__(self, concerns_dir: str | None = None):
    ...
    def _load_pack(self, filepath: str) -> ConcernPack | None:
        """加载单个增强包文件"""
        try:
            import yaml
        except ImportError:
    ...
    def _load_pack_simple(self, filepath: str) -> ConcernPack | None:
    ...
    def list_available_packs(self) -> list[dict[str, str]]:
        ...
        self._load_all_packs()
        return [
            {"id": pack.concern_id, "name": pack.name, "description": pack.description} for pack in self._packs.values()
        ]
