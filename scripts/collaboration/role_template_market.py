...
class RoleTemplateMarket:
    ...
    def _save_to_disk(self):
        ...
        try:
            ...
            templates_file.write_text(json.dumps(templates_data, indent=2, ensure_ascii=False), encoding="utf-8")

            ratings_file = self.storage_dir / "ratings.json"
            all_ratings = []
            for ratings_list in self._ratings.values():
                all_ratings.extend([asdict(r) for r in ratings_list])
            ratings_file.write_text(json.dumps(all_ratings, indent=2, ensure_ascii=False), encoding="utf-8")
        except Exception as e:
    ...
    def search(
        self, query: str = "", category: str = "", tags: list[str] = None, limit: int = 20
    ) -> list[RoleTemplate]:
        """
        Search templates by keyword, category, or tags.
    ...
    def install(self, template_id: str) -> RoleTemplate | None:
        """
        ...
        install_file.write_text(json.dumps(tpl.to_dict(), indent=2, ensure_ascii=False), encoding="utf-8")

        logger.info("Installed template: %s (%s)", tpl.name, template_id)
        return tpl

    def rate(self, template_id: str, user_id: str, score: float, comment: str = "") -> bool:
        """
        Rate a template.
    ...
    def get_popular(self, limit: int = 10) -> list[RoleTemplate]:
        ...
        sorted_tpls = sorted(self._templates.values(), key=lambda t: t.install_count, reverse=True)
    ...
    def export_template(self, template_id: str, output_path: str) -> bool:
        ...
        if not tpl:
            return False
        try:
            Path(output_path).write_text(json.dumps(tpl.to_dict(), indent=2, ensure_ascii=False), encoding="utf-8")
            return True
        except Exception as e:
            logger.warning("Export failed: %s", e)
            return False

    def import_template(self, input_path: str) -> str | None:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_rating": round(sum(t.rating for t in self._templates.values()) / len(self._templates), 1)
            if self._templates
            else 0.0,
        }
