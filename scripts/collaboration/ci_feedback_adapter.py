...
class CoverageParser:
    ...
    def parse(output: str) -> CIResult:
        ...
        if line_match:
            ...
            result.metrics.append(CIMetric("lines_missed", missed, "warning" if missed > 10 else "pass"))
...
class LintParser:
    ...
    def parse(output: str) -> CIResult:
        ...
        result.metrics.append(CIMetric("errors", error_count, "fail" if error_count > 0 else "pass"))
        result.metrics.append(CIMetric("warnings", warning_count, "warning" if warning_count > 0 else "pass"))
...
class BuildParser:
    ...
    def parse(output: str) -> CIResult:
        ...
        result.metrics.append(
            CIMetric("build_status", "success" if result.success else "failed", "pass" if result.success else "fail")
        )
...
class CIFeedbackAdapter:
    ...
    def parse_ci_output(self, output: str, source_type: str) -> CIResult | None:
        """
    ...
    def generate_context(self, results: list[CIResult]) -> CIContext:
        ...
        context.quality_gates["build_succeeds"] = all(r.success for r in results if r.source == "build") or not any(
            r.source == "build" for r in results
        )
