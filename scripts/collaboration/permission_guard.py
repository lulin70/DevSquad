...
def _build_default_rules() -> list[PermissionRule]:
    return [
        # File Read (low risk)
        PermissionRule("R001", ActionType.FILE_READ, "**/*", PermissionLevel.PLAN, "Read any file", risk_boost=0.0),
        # File Create (medium risk)
        PermissionRule(
            "R002", ActionType.FILE_CREATE, "*.py", PermissionLevel.AUTO, "Create Python file", risk_boost=0.1
        ),
        PermissionRule(
            "R003", ActionType.FILE_CREATE, "*.md", PermissionLevel.AUTO, "Create markdown doc", risk_boost=0.05
        ),
        PermissionRule(
            "R004", ActionType.FILE_CREATE, "*.json", PermissionLevel.DEFAULT, "Create JSON data file", risk_boost=0.15
        ),
        PermissionRule(
            "R005", ActionType.FILE_CREATE, "*", PermissionLevel.DEFAULT, "Create other file type", risk_boost=0.2
        ),
        # File Modify (medium-high risk)
        PermissionRule(
            "R006", ActionType.FILE_MODIFY, "*.py", PermissionLevel.AUTO, "Modify Python source", risk_boost=0.2
        ),
        PermissionRule("R007", ActionType.FILE_MODIFY, "*.md", PermissionLevel.AUTO, "Modify document", risk_boost=0.1),
        PermissionRule(
            "R008", ActionType.FILE_MODIFY, "*.json", PermissionLevel.DEFAULT, "Modify config file", risk_boost=0.25
        ),
        PermissionRule(
            "R009", ActionType.FILE_MODIFY, ".env*", PermissionLevel.BYPASS, "Modify env file", risk_boost=0.8
        ),
        PermissionRule(
            "R010",
        ...
        PermissionRule(
            "R011", ActionType.FILE_MODIFY, "*", PermissionLevel.DEFAULT, "Modify other files", risk_boost=0.25
        ),
        ...
        PermissionRule(
            "R013", ActionType.FILE_DELETE, "*.pyc", PermissionLevel.AUTO, "Delete compiled cache", risk_boost=0.1
        ),
        PermissionRule(
            "R014", ActionType.FILE_DELETE, ".git/**", PermissionLevel.BYPASS, "Delete Git dir content", risk_boost=0.99
        ),
        PermissionRule("R015", ActionType.FILE_DELETE, "*", PermissionLevel.BYPASS, "Delete any file", risk_boost=0.9),
        # Shell Execute (very high risk)
        PermissionRule(
            "R016", ActionType.SHELL_EXECUTE, "cat *", PermissionLevel.AUTO, "View file content", risk_boost=0.05
        ),
        PermissionRule(
            "R017", ActionType.SHELL_EXECUTE, "ls *", PermissionLevel.AUTO, "List directory", risk_boost=0.05
        ),
        PermissionRule(
            "R018", ActionType.SHELL_EXECUTE, "git *", PermissionLevel.AUTO, "Git read-only commands", risk_boost=0.1
        ),
        PermissionRule(
            "R019",
        ...
        PermissionRule(
            "R020", ActionType.SHELL_EXECUTE, "rm *", PermissionLevel.BYPASS, "Remove command", risk_boost=0.95
        ),
        PermissionRule(
            "R021", ActionType.SHELL_EXECUTE, "sudo *", PermissionLevel.BYPASS, "Privilege escalation", risk_boost=1.0
        ),
        PermissionRule(
            "R022", ActionType.SHELL_EXECUTE, "*", PermissionLevel.DEFAULT, "Other shell commands", risk_boost=0.6
        ),
        # Network Request (medium-high risk)
        PermissionRule(
            "R023", ActionType.NETWORK_REQUEST, "*pypi.org*", PermissionLevel.AUTO, "PyPI download", risk_boost=0.15
        ),
        PermissionRule(
            "R024",
        ...
        PermissionRule(
            "R025", ActionType.NETWORK_REQUEST, "*", PermissionLevel.DEFAULT, "Other network requests", risk_boost=0.5
        ),
        # Git Operations
        PermissionRule(
            "R026", ActionType.GIT_OPERATION, "*", PermissionLevel.AUTO, "Git read operations", risk_boost=0.05
        ),
        PermissionRule(
            "R027",
        ...
        PermissionRule(
            "R028", ActionType.GIT_OPERATION, "*push*", PermissionLevel.DEFAULT, "Git push operation", risk_boost=0.4
        ),
        PermissionRule(
            "R029",
            ActionType.GIT_OPERATION,
        ...
        # Environment
        PermissionRule(
            "R030", ActionType.ENVIRONMENT, "*", PermissionLevel.BYPASS, "Modify environment vars", risk_boost=0.7
        ),
    ]
...
_SENSITIVE_PATH_PATTERNS_COMPILED = [re.compile(p, re.IGNORECASE) for p in _SENSITIVE_PATH_PATTERNS]
...
class PermissionGuard:
    ...
    def __init__(
        ...
        self._rule_index: dict[str, int] = {r.rule_id: i for i, r in enumerate(self.rules)}
    ...
    def _match_rule(self, action: ProposedAction) -> PermissionRule | None:
        best_match = None
        best_strictness = -1
    ...
    def _dim_source_trust(self, role_id: str | None) -> float:
        known_roles = {"architect", "product-manager", "tester", "solo-coder", "ui-designer", "devops"}
    ...
    def _make_decision(self, action: ProposedAction, outcome: DecisionOutcome, reason: str) -> PermissionDecision:
    ...
    def get_audit_log(
        ...
        if action_type:
            results = [e for e in results if e.action and e.action.action_type == action_type]
        if outcome:
            results = [e for e in results if e.decision and e.decision.outcome == outcome]
        if worker_id:
            results = [e for e in results if e.action and e.action.source_worker_id == worker_id]
