        ...
        with self._lock:
            ...
            for iteration in range(self._max_iterations + 1):
                ...
                logger.info(
                    ...
                    iteration + 1, self._max_iterations + 1, current_task
                )
                ...
                if dry_run:
                    ...
                    result = self._dispatcher.dispatch(
                        current_task, roles=roles, mode=mode, **kwargs
                    )
                ...
                logger.info(
                    ...
                    iteration + 1, quality, self._quality_gate,
                ...
                if quality >= self._quality_gate:
                    logger.info(
                        "Quality gate met at iteration %d (%.2f >= %.2f)",
                        iteration + 1, quality, self._quality_gate
                    )
                    break
                ...
                if iteration < self._max_iterations:
                    ...
                    logger.info(
                        "Generated adjustment for iteration %d: %.80s...",
                        iteration + 1, adjustment
                    )
    ...
    def _generate_adjustment(self, result, quality: float) -> str:
        ...
        if failed_workers:
            ...
            if len(failed_roles) == 1:
                ...
                adjustments.append(f"Address failures in roles: {roles_str}. Consider breaking down task into smaller sub-tasks")
        ...
        if successful_workers and failed_workers:
            ...
            if success_count / total < 0.5:
                adjustments.append("Low success rate detected. Consider reducing number of roles or simplifying task scope")

        if quality < 0.4:
            adjustments.append("Quality critically low. Recommend complete task reformulation with clearer objectives")
        elif quality < 0.6:
            adjustments.append("Quality below acceptable level. Strengthen task description with examples and constraints")

        if not adjustments:
            adjustments.append("General refinement needed: Review outputs against original goals and add missing requirements")
        ...
        with self._lock:
            ...
            return {
                ...
                "converged_at_iteration": next(
                    (h["iteration"] for h in self._iteration_history if h["passed"]),
                    None
                ),
